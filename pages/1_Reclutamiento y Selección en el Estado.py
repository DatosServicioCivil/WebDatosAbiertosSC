...
ministerios=pd.read_excel('datos/ministerios.xlsx',sheet_name='Hoja1')
...
if a=='Empleo Público':
    ...
    if seleccion_eepp=='Postulaciones':
        ...
        if option_S1=='Todos' and option_S2=='Todos' and option_S3=='Todos' and option_S4=='Todos': #1
            ...
            if option_S1!='Todos' and option_S2=='Todos' and option_S3=='Todos' and option_S4!='Todos': #10
                filtro=(df_postulaciones_eepp['Region_Homologada']==option_S1) & (df_postulaciones_eepp['Sexo']==option_S4)    
            if option_S1!='Todos' and option_S2=='Todos' and option_S3!='Todos' and option_S4=='Todos': #11
                filtro=(df_postulaciones_eepp['Region_Homologada']==option_S1) & (df_postulaciones_eepp['Ministerio']==option_S2) 
        ...
        graf2.update_yaxes(visible=visible_y_axis,title_text=None)
        ...
        graf3.update_yaxes(visible=visible_y_axis,title_text=None)
        graf3.update_layout(yaxis_tickformat='.0f')
    ...
    if seleccion_eepp=='Seleccionados': 
        ...
        def seleccionados_eepp():
            ...
            df_sel_eepp=df_sel_eepp.drop(columns=['Region','region_postulante'])
            return df_sel_eepp
    ...
    if seleccion_pch=='Postulaciones':
        ...
        graf11=px.bar(postulaciones_x_ministerio_pch,x='Ministerio',y='Postulaciones',title='<b>Postulaciones por Ministerio</b>',color_discrete_sequence=[color_bar]).\
             update_yaxes(visible=visible_y_axis,title_text=None).\
                     update_xaxes(title_text=None,tickmode='linear', dtick=1,tickangle=-90)
        #----------------------------------------------------------------------------------------------------------------------------  
        graf12=px.bar(postulaciones_x_año_estado, x="Año", y="Postulaciones",color='EstadoPost',color_discrete_map=estados_pch_color_map ,title="Estado de Postulaciones por año").\
             update_yaxes(visible=visible_y_axis,title_text=None).\
                    update_xaxes(title_text=None,tickmode='linear', dtick=1).\
                        update_layout(legend=dict(x=0.5, xanchor='center', y=-0.1, yanchor='top', traceorder='normal', itemsizing='trace',orientation='h'))  # Ubicar debajo del eje x en dos columnas
