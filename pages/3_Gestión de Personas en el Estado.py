...
if a=='Capacitación en el Estado':
    ...
    if option_1=='Todos' and option_2=='Todos' and option_3=='Todos' and option_4=='Todos': #1
        ...
        Inversion=df_actividades_ejecutadas_sispubli[filtro].groupby('Año').agg({'Gasto_monto_Item001':'sum'}).reset_index()
        Participantes=df_actividades_ejecutadas_sispubli[filtro].groupby('Año').agg({'Numero_de_Participantes':'sum'}).reset_index()
        Metodologia_Actividades=df_actividades_ejecutadas_sispubli[filtro].groupby(['Año','Metodología_de_Aprendizaje']).agg({'id_actividad':'sum'}).reset_index()
        Metodologia_Participantes=df_actividades_ejecutadas_sispubli[filtro].groupby(['Año','Metodología_de_Aprendizaje']).agg({'Numero_de_Participantes':'sum'}).reset_index()
